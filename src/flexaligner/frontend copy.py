...
class TextFrontend:
    ...
    def get_phonemes(self, text: str, lang: str) -> List[str]:
        ...
        if lang == "en":
            ...
            for token in raw_tokens:
                # --- 维度 A: 闭集探针 (优先判定是否已经是合法音素) ---
                # 因为 clean_text 强制转了小写，我们需要恢复大写去撞击 dict2
                token_upper = token.upper()
                token_lower = token.lower()
                
                # 绝对优先：如果在 dict2 缓存里 (如 'IY1', 'SH', 'sil')，直接放行
                if token_upper in self._vocab_cache:
                    final_tokens.append(token_upper)
                elif token_lower in self._vocab_cache:
                    final_tokens.append(token_lower)
                elif token in self._vocab_cache:
                    final_tokens.append(token)
                else:
                    # --- 维度 B: G2P 就地粉碎 ---
                    # 字典里没有，说明它是未知的野生单词 (如 'we', 'huh', 'and')
                    if self.config.get("use_g2p", True):
                        # 启动 G2P 翻译成音素列表，如 "huh" -> ['HH', 'AH1']
                        phonemes = self.g2p(token)
                        for p in phonemes:
                            # 剔除 G2P 可能残留的单引号等标点
                            p_clean = ''.join([c for c in p if c.isalnum()]).strip()
                            if p_clean:
                                # 保持音素的物理维度（大写带数字）进入下游
                                final_tokens.append(p_clean.upper())
                    else:
                        # 没开 G2P，只能原样保留（大概率会在下游抛出 OOV）
                        final_tokens.append(token)
